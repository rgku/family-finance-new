...
BEGIN
  ...
  RETURN replace(
    encode(pgp_sym_encrypt(value::TEXT, 'REMOVED_ENCRYPTION_KEYFeGW0Hbw8pWIyfezBOk='), 'base64'),
    E'\n', ''
  );
END;
...
BEGIN
  ...
  RETURN pgp_sym_decrypt(decode(clean_value, 'base64'), 'REMOVED_ENCRYPTION_KEYFeGW0Hbw8pWIyfezBOk=')::DECIMAL;
...
BEGIN
  IF value IS NULL THEN RETURN NULL; END IF;
  RETURN replace(
    encode(pgp_sym_encrypt(value, 'REMOVED_ENCRYPTION_KEYFeGW0Hbw8pWIyfezBOk='), 'base64'),
    E'\n', ''
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  IF value IS NULL THEN RETURN NULL; END IF;
  clean_value := replace(replace(value, E'\n', ''), E'\r', '');
  RETURN pgp_sym_decrypt(decode(clean_value, 'base64'), 'REMOVED_ENCRYPTION_KEYFeGW0Hbw8pWIyfezBOk=');
